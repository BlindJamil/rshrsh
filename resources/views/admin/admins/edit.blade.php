                             ...
                             @forelse ($roles as $role)
                                <label class="flex items-center">
                                    <input type="checkbox" name="roles[]" value="{{ $role->id }}"
                                           ...
                                           {{-- Check if role was previously assigned OR if it's in old input (validation error) --}}
                                           {{ in_array($role->id, old('roles', $assignedRoleIds)) ? 'checked' : '' }}
                                           {{-- Prevent unchecking super_admin role if this is the logged-in super admin --}}
                                    ...
                                    <span class="text-xs text-gray-400 ml-2">({{ $role->name }})</span>
                                     {{-- Add visual cue if role cannot be changed --}}
