...
<div class="bg-gray-900 min-h-screen text-white p-8">
    <h1 class="text-3xl font-bold mb-6">Edit Cause</h1>
    ...
    <div class="bg-gray-800 shadow-md rounded-lg p-6">
        <form action="{{ route('admin.causes.update', $cause->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                <div class="flex flex-col space-y-2">
                    <label class="inline-flex items-center">
                        <input type="radio" name="cause_type" value="general" {{ old('cause_type', $cause->is_recent ? 'recent' : 'general') === 'general' ? 'checked' : '' }} class="text-yellow-500 bg-gray-700 border-gray-600 focus:ring-yellow-500" id="general_cause">
                        <span class="ml-2 text-gray-300">General Cause</span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="radio" name="cause_type" value="recent" {{ old('cause_type', $cause->is_recent ? 'recent' : 'general') === 'recent' ? 'checked' : '' }} class="text-yellow-500 bg-gray-700 border-gray-600 focus:ring-yellow-500" id="recent_cause">
            ...
            <div class="mb-6 ml-6" id="urgent_option" style="{{ old('cause_type', $cause->is_recent ? 'recent' : 'general') === 'recent' ? '' : 'display: none;' }}">
                <label class="flex items-center">
                    <input type="checkbox" name="is_urgent" {{ old('is_urgent', $cause->is_urgent) ? 'checked' : '' }} class="mr-2 rounded bg-gray-700 border-gray-600 text-red-400 focus:ring-red-500">
